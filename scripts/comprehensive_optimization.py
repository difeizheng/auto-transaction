"""
综合策略优化 - 冲击 15% 年化目标
在移动 15% 触发 (13.20%) 基础上进一步优化

优化方向:
1. 信号阈值：5.5, 6.0, 6.5 (更严格过滤)
2. 止盈：38%, 40%, 45% (更宽止盈)
3. 仓位：35%, 40%, 45% (牛市仓位)
4. 止损：3.5%, 4.0% ( tighter stop)
"""
import sys
...
def test_comprehensive_optimization():
    ...
    test_configs = [
        # 基准：移动 15% 触发
        {'name': '基准 (移动 15%)', 'threshold': 5.0, 'take_profit': 0.35, 'max_pos': 0.45, 'stop_loss': 0.04},
        # 测试更高阈值
        {'name': '阈值 5.5', 'threshold': 5.5, 'take_profit': 0.35, 'max_pos': 0.45, 'stop_loss': 0.04},
        {'name': '阈值 6.0', 'threshold': 6.0, 'take_profit': 0.35, 'max_pos': 0.45, 'stop_loss': 0.04},
        # 测试更宽止盈
        {'name': '止盈 38%', 'threshold': 5.0, 'take_profit': 0.38, 'max_pos': 0.45, 'stop_loss': 0.04},
        {'name': '止盈 40%', 'threshold': 5.0, 'take_profit': 0.40, 'max_pos': 0.45, 'stop_loss': 0.04},
        {'name': '止盈 45%', 'threshold': 5.0, 'take_profit': 0.45, 'max_pos': 0.45, 'stop_loss': 0.04},
        # 组合优化 A: 高阈值 + 宽止盈
        {'name': '组合 A (阈值 5.5+ 止盈 40%)', 'threshold': 5.5, 'take_profit': 0.40, 'max_pos': 0.45, 'stop_loss': 0.04},
        {'name': '组合 B (阈值 6.0+ 止盈 45%)', 'threshold': 6.0, 'take_profit': 0.45, 'max_pos': 0.45, 'stop_loss': 0.04},
        # 组合优化 B: 更紧止损
        {'name': '组合 C (阈值 5.5+ 止损 3.5%)', 'threshold': 5.5, 'take_profit': 0.35, 'max_pos': 0.45, 'stop_loss': 0.035},
        {'name': '组合 D (阈值 5.0+ 止损 3.5%)', 'threshold': 5.0, 'take_profit': 0.35, 'max_pos': 0.45, 'stop_loss': 0.035},
        # 激进组合：高阈值 + 宽止盈 + 紧止损
        {'name': '激进 A (阈值 5.5+ 止盈 40%+ 止损 3.5%)', 'threshold': 5.5, 'take_profit': 0.40, 'max_pos': 0.45, 'stop_loss': 0.035},
        {'name': '激进 B (阈值 6.0+ 止盈 45%+ 止损 3.5%)', 'threshold': 6.0, 'take_profit': 0.45, 'max_pos': 0.45, 'stop_loss': 0.035},
        # 更高牛市仓位
        {'name': '牛市 50%', 'threshold': 5.0, 'take_profit': 0.35, 'max_pos': 0.50, 'stop_loss': 0.04},
        {'name': '牛市 55%', 'threshold': 5.0, 'take_profit': 0.35, 'max_pos': 0.55, 'stop_loss': 0.04},
    ]
    ...
    for i, cfg in enumerate(test_configs, 1):
        ...
        params = OptimalStrategyParams(
            ...
            trailing_stop_trigger=0.15,  # 固定 15% 触发
    ...
    print(f"\n最优策略 {best['name']}: {status_str}")
    print(f"  年化：{best['annual']*100:.2f}% (目标：≥15%)")
    print(f"  夏普：{best['sharpe']:.2f} (目标：≥1.0)")
    print(f"  胜率：{best['win_rate']*100:.1f}% (目标：≥55%)")
    print(f"  回撤：{best['drawdown']*100:.2f}% (目标：≤15%)")

    # Top 5 推荐
    print("\n" + "=" * 80)
    print("Top 5 策略推荐")
    print("=" * 80)
    for idx, (_, r) in enumerate(df_sorted.head(5).iterrows(), 1):
        print(f"  #{idx} {r['name']}")
